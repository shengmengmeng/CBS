...
def uniform_mix_C(mixing_ratio, num_classes, num_list):
    ...
    return C
def flip_labels_C_two(corruption_prob, num_classes, num_list):
    '''
    ...
    # for i in range(num_classes):
    #     idx=np.random.choice(row_indices[row_indices != i], 2, replace=False)
    #     C[i][idx] = corruption_prob * num_cls[idx]/sum(num_cls[idx])
    # return C
    '''
        returns a matrix with (1 - corruption_prob) on the diagonals, and corruption_prob
        concentrated in only one other entry for each row
...
class CIFAR10_im(data.Dataset):
        ...
        if self.train:
            ...
            if meta is True:
                ...
                if corruption_type == 'unif':
                    ...
                    print("flip2")
                    C = flip_labels_C_two(self.corruption_prob, num_classes, img_imblance_num_list)
                    # if local_rank == 0:
                    #    print(C)
                    self.C = C
                else:
                ...
                if corruption_type == 'imb':
                    noise_label = []
                    num_train = len(self.train_labels)
                    idx = list(range(num_train))
                    random.shuffle(idx)
                    cls_num_list = img_imblance_num_list

                    num_noise = int(self.corruption_prob * num_train)
                    noise_idx = idx[:num_noise]

                    print("imb")
                    p = np.array([cls_num_list for _ in range(num_classes)])
                    for i in range(num_classes):
                        p[i][i] = 0
                    ...
                    for i in range(num_train):
                        if i in noise_idx:
                            newlabel = np.random.choice(num_classes, p=p[self.train_labels[i]])
                            assert newlabel != self.train_labels[i]
                ...
                for i in range(len(self.train_labels)):
                    if self.train_labels[i] == self.true_labels[i]:
                        a+=1
                self.noise_or_not=[int(self.train_labels[i] == self.true_labels[i]) for i in range(len(self.true_labels))]
                # print(len(self.noise_or_not))
    ...
    def __getitem__(self, index):
        if self.train:
            img, target, true_target, noise_or_not = self.train_data[index], self.train_labels[index], self.true_labels[index], self.noise_or_not[index]
        else:
            img, target, true_target, noise_or_not = self.test_data[index], self.test_labels[index], self.test_labels[index], self.test_labels[index]
