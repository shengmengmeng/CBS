...
class NoisyCIFAR100_class_imbalanced(torchvision.datasets.CIFAR100):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False,
                 noise_type='clean', closeset_ratio=0.0, openset_ratio=0.2, random_state=0, verbose=True):
        super().__init__(root, train=train, transform=transform, target_transform=target_transform,
                         download=download)
        ...
        num_samples = len(self.data)
        if self.train and (noise_type != 'clean'):
            print("train_data before:" + str(len(self.data)))
            data_list_val = {}
            for j in range(100):
                data_list_val[j] = [i for i, label in enumerate(self.targets) if label == j]
            ...
            self.data = self.data[idx_to_train]
            self.targets = list(np.array(self.targets)[idx_to_train])
            print("train_data after:" + str(len(self.data)))
