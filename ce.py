...
def get_label_txt(net, test_data, dev, to_embeds=None):
    ...
    with torch.no_grad():
        if to_embeds is None:
            data = torch.from_numpy(test_data).to(dev)
        else:
            data = torch.from_numpy(to_embeds(test_data)).to(dev)
        outputs = net(data)
        if type(outputs) is dict:
            outputs = outputs['logits']
...
def warmup(net, optimizer, trainloader, dev, train_loss_meter,train_accuracy_meter, no_penalty=False, is_sst=False):
    if isinstance(trainloader, dict):
        warmup_txt(net, optimizer, trainloader, dev,train_loss_meter,train_accuracy_meter, no_penalty, is_sst)
    else:
        warmup_img(net, optimizer, trainloader, dev,train_loss_meter,train_accuracy_meter, no_penalty)


def warmup_img(net, optimizer, trainloader, dev,train_loss_meter,train_accuracy_meter, no_penalty=False):
    net.train()
...
def warmup_txt(net, optimizer, trainloader, dev,train_loss_meter,train_accuracy_meter, no_penalty=False, is_sst=False):
    net.train()
    ...
    for i in range(num_batches):
        ...
        x, y = torch.from_numpy(x_batch).to(dev), torch.from_numpy(y_batch).to(dev)
        outputs = net(x)
        logits = outputs['logits'] if type(outputs) is dict else outputs
        loss_ce = F.cross_entropy(logits, y)
        if no_penalty:
...
def eval_train(net, num_classes, num_samples, trainloader, clean_rate, dev, targets_list=None, criterion='jsdiv'):
    ...
    logits_ = torch.zeros(num_samples, num_classes).to(dev)
    with torch.no_grad():
        if not is_text_task:
            for it, sample in enumerate(trainloader):
                ...
                inputs, y = inputs.to(dev), y.to(dev)
                outputs = net(inputs)
                logits = outputs['logits'] if type(outputs) is dict else outputs
                if criterion == 'loss':
    ...
    return prob_clean, targets_list, logits_


def robust_train(net, optimizer, trainloader, dev, train_loss_meter,train_accuracy_meter, aug, fmix, num_class, ep, p_clean, logits_, loss_weight, corr_label_dist, ema_label_dist, params, is_sst=False):
    if isinstance(trainloader, dict):
        robust_train_txt(net, optimizer, trainloader, dev, train_loss_meter,train_accuracy_meter, aug, fmix, num_class, ep, p_clean, logits_, loss_weight, corr_label_dist, ema_label_dist, params, is_sst)
    else:
        robust_train_img(net, optimizer, trainloader, dev, train_loss_meter,train_accuracy_meter, aug, fmix, num_class, ep, p_clean, logits_, loss_weight, corr_label_dist, ema_label_dist, params)


def robust_train_img(net, optimizer, trainloader, dev, train_loss_meter,train_accuracy_meter, aug, fmix, num_class, ep, p_clean, logits_, loss_weight, corr_label_dist, ema_label_dist, params):
    net.train()
    ...
    for it, sample in enumerate(trainloader):
        ...
        batch_size = x.size(0)
        x, y = x.to(dev), y.to(dev)
        labels_x = torch.zeros(batch_size, num_class).to(dev).scatter_(1, y.view(-1, 1), 1)
        w_x = torch.FloatTensor(p_clean[indices]).view(-1, 1).to(dev) if isinstance(p_clean, np.ndarray) else p_clean[indices].view(-1, 1)
        ...
        with torch.no_grad():
            ...
            if params.useEMA:
                label_ema = ema_label_dist[indices, :].clone().to(dev)
        ...
        if idx_noises.size(0)>0:
            x2 = aug(x[idx_noises], mode='s').to(dev)
            outputs2 = net(x2)
...
def robust_train_txt(net, optimizer, trainloader, dev, aug, fmix, num_class, ep, p_clean, logits_, loss_weight, corr_label_dist, ema_label_dist, params, is_sst=False):
    net.train()
...
def get_img_size(dataset):
    if dataset in ['cifar10', 'cifar100','cross_model_data']:
        return (32, 32)
    elif dataset in ['tiny_imagenet', 'mix_data']:
        return (56, 56)
...
def parse_args():
    ...
    parser.add_argument('--task', type=str, default='5-0')
    parser.add_argument('--logger-root', type=str, default='./result/')
    parser.add_argument('--log', type=str, default='ProMix')
    ...
    parser.add_argument('--lr-decay', type=str, default='cosine:10,5e-4,30')
    ...
    parser.add_argument('--aug-type', type=str, default='rand-1')
    ...
    parser.add_argument('--rho-range', type=str, default='0.7:0.6:15', help='Format: stop:start:step')
    parser.add_argument('--omega-range', type=str, default='1.0:1.0:15', help='Format: stop:start:step')
    parser.add_argument('--tau', type=float, default=0.9)
    parser.add_argument('--tau-expand', type=float, default=0.8)
    parser.add_argument('--start-expand', type=int, default=2000)
...
if __name__ == '__main__':
    ...
    if task_id =='6':
        ...
        task,sub_task=config.task.split('-')
        sys.path.append('../task{}'.format(task))
        from dataloader_webvision_all import *
        from InceptionResNetV2 import *

    else:
        task,sub_task=config.task.split('-')
        sys.path.append('../task{}/data{}'.format(task,sub_task))
        from model import *
    ...
    for epoch in range(0, config.epochs):
        ...
        if epoch < config.warmup_epochs:
            warmup(model, optim, input_loader, device, train_loss_meter,train_accuracy_meter, config.no_penalty, dataset_name=='sst')
        else:
            ...
            robust_train(model, optim, input_loader, device, train_loss_meter,train_accuracy_meter, re_aug, fmix_helper, n_classes, epoch, approx_clean_probs, logits_, omega,
    ...
    #     test_pred_results = get_label(model, loader_dict['web_valloader'], device)
    #     test_pred_results = np.array(test_pred_results.cpu())
    #     np.save(f'{result_dir}/label_test_web.npy', test_pred_results)
    #     test_pred_results = get_label(model, loader_dict['imagenet_valloader'], device)
    #     test_pred_results = np.array(test_pred_results.cpu())
    #     np.save(f'{result_dir}/label_test_img.npy', test_pred_results)
    # else:
    #     if dataset_name in ['sst', 'twitter']:
    #         test_pred_results = get_label_txt(model, loader_dict['test_loader'], device,
    #                                           to_embeds=loader_dict['to_embeds'])
    #     else:
    #         test_pred_results = get_label(model, loader_dict['test_loader'], device)
    #     test_pred_results = np.array(test_pred_results.cpu())
    #     np.save(f'{result_dir}/label_test.npy', test_pred_results)
    ...
    # elapse = time.time() - T1
    # gpu_max_mem = torch.cuda.max_memory_allocated()
    # print(f'Total time elapse: {elapse:.2f} seconds; Max GPU Memory Allocated: {gpu_max_mem/(2**30):.2f} Gb ({gpu_max_mem/(2**10):.2f} Kb)')
